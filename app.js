...
const updateData = () => {
  dataLocaStorage = JSON.parse(localStorage.getItem('transactions')) || [];

  currentID = dataLocaStorage.shift()?.currentID || 0;
  transactions = dataLocaStorage !== null ? dataLocaStorage : [];
};
...
const getBalance = (transactionsAmounts) => transactionsAmounts.reduce((acc, amount) => acc + amount, 0).toFixed(2);
...
const handleSubmit = (event) => {
  ...
  const inputName = event.target.querySelector('#transaction-name');
  const inputAmount = event.target.querySelector('#transaction-amount');
